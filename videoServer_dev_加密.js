...
app.post('/video/liveBroadcast', jsonParser, function (req, res) {
  ...
  // 判断前端是否发送需要的数据
  if (number === undefined || number.includes("，") || sessionId === undefined || type >= 3 || type < 1) {
    res.send(
      {
        "code": 0,
        "msg": "操作失败",
...
app.put('/video/quit', jsonParser, function (req, res) {
  ...
  deviceNumberList.forEach(element => {
    ...
    global.sessionIdDeviceMap.forEach((value, key) => {
      ...
      // console.log("下线设备：global.sessionIdDeviceMap.get(key)"  + global.sessionIdDeviceMap.get(key));
...
var server = app.listen(65500, function () {
  ...
  global.keyTest = CryptoJS.enc.Utf8.parse("SONTECH-TEXPRO"); 
//   global.keyTest = CryptoJS.enc.Utf8.parse("1234123412ABCDEF"); 
  global.keyIv = CryptoJS.enc.Utf8.parse('ABCDEF1234123412'); 

  // 这里定义设备信息
  // global.deviceRun存放设备的编号和进程的对应关系，标清
