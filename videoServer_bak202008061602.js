...
var server = app.listen(65500, function () {
  ...
  setInterval(() => {
    ...
    global.deviceIdAndTime.forEach((value, key) => {
      // console.log("这是定时清理设备里面的时间：" + myDate.toLocaleTimeString());
      // if(key == 1){
      //   console.log("key： " + key);
      //   console.log("这个是定时清理的global.deviceIdAndTime: " + global.deviceIdAndTime.get(key));
      // }
      console.log(global.deviceIdAndTime.get(key));
      if ((myDate.getTime() - value) / 1000 / 60 > 2) {
        var deviceKey = key;
        // 杀session里的设备
        global.sessionIdDeviceMap.forEach((value, key) => {
